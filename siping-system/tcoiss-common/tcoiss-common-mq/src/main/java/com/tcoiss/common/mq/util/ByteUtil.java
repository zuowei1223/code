...
public class ByteUtil {
     ...
     * @param index 
     */  
    public static byte[] putInt(int num) {  
     ...
     * @return 
     */  
    public static int getInt(byte[] byteNum) {  
        int num = 0;  
        for (int ix = 0; ix < 4; ++ix) {  
            num <<= 8;  
            num |= (byteNum[ix] & 0xff);  
     ...
     * @param index 
     */  
    public static byte[] putLong( long num) {  
     ...
     * @return
     */
    public static MdmMessage byteToMsg(byte[] bytes) {
        try {
            String messageBody = new String(bytes, RemotingHelper.DEFAULT_CHARSET);
            JSONObject msgJson = JSONObject.parseObject(messageBody);
            //BaseMqDTO dto = JSON.toJavaObject(msgJson , BaseMqDTO.class);
            MdmMessage msg = new MdmMessage();
            msg.setJsonObject(msgJson);
            msg.setObjId(msgJson.getString("id"));
            msg.setOperMethod(OperMethodEnum.UPDATE);
            return msg;
    ...
    public static int htonl(int d){
        int rslt = 0;
        byte [] bs1 = ByteUtil.putInt(d);
        byte[] bs2 = ReversEndian(bs1, 4, false);
        rslt = ByteUtil.getInt(bs2);
        return rslt;
    }
    
    public static void main(String[] args)    
    {
        String msg = "{\"birthday\":\"1991-10-08 00:00:00.000\",\n" +
                "\"creator\":\"1079513901072137216\",\n" +
                "\"createtime\":\"2021-04-07 10:07:23.000\",\n" +
                "\"gender\":\"1\",\n" +
                "\"modifytime\":\"2021-04-08 16:47:10.000\",\n" +
                "\"modifier\":\"1079513901072137216\",\n" +
                "\"number\":\"ID-000113\",\n" +
                "\"phone\":\"15279115047\",\n" +
                "\"enable\":\"1\",\n" +
                "\"idcard\":\"362228199110083137\",\n" +
                "\"name\":{\"zh_CN\":\"测试修改\"},\n" +
                "\"id\":\"1128782828033562624\",\n" +
                "\"$id\":\"1128782828033562624\",\n" +
                "\"$pk\":\"id\",\"status\":\"C\"}";
        JSONObject msgJson = JSONObject.parseObject(msg);
        BaseMqDTO dto = JSON.toJavaObject(msgJson , BaseMqDTO.class);
        String dtoStr= JSONObject.toJSONString(dto);
        PersonDto personDto = JSONObject.parseObject(dtoStr,PersonDto.class);
